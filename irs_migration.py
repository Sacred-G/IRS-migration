...
im = Image.open('Data/content/migration.png')
...
summary = st.sidebar.radio(
    'Summary level:',
    ('11-County Atlanta Metro', 'Single County')
    )
...
summary_dict = {
    '11-County Atlanta Metro':'metro Atlanta',
...
image = Image.open('Data/content/logo.png')
...
st.sidebar.text("")
st.sidebar.text("")
st.sidebar.caption('*Source: [IRS Statistics of Income](https://www.irs.gov/statistics/soi-tax-stats-migration-data)*')

# main content -------------------------------------------------------------------------------------------------

# set header
st.header(f'Atlanta Migration Dashboard | {years[0]} - {years[-1]}')
...
def dollars_person_map():
    ...
    gdf = gpd.read_file('Data/ARC_counties.geojson')
